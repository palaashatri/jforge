...
- [x] `GenericOnnxService` split into per-architecture pipelines (in progress) — CLIP + T5 tokenizers extracted to public reusable `tokenize` package; scheduler/latent math delegated to shared `engine.scheduler.SchedulerMath` and `engine.random.LatentNoise` (identical formulas, byte-for-byte verified); SD3 shifted-sigma loop now uses `FlowMatchEulerScheduler.shifted()`. The `run*` inference methods remain centralized — full per-architecture class extraction is deferred to preserve working behavior.
