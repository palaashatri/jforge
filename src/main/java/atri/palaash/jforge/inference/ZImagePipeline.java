...
public final class ZImagePipeline implements GenerationPipeline {
    @Override public PipelineDescriptor descriptor() { return new PipelineDescriptor("z-image", "Z-Image", "zimage", 1); }
    @Override public PipelineCapabilities capabilities() {
        return PipelineCapabilities.builder().capabilities(Capability.TEXT_TO_IMAGE, Capability.LORA).build();
    }
    ...
    @Override public GenerationResult generate(LoadedPipeline p, GenerationRequest req, ProgressListener prog, CancellationToken cancel) {
        return GenerationResult.fail(req.modelId(), "Z-Image pipeline validated — execution pending");
    }
}
